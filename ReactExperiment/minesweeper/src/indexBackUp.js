    ...
    constructor(width, height, mineTotal = 10){
        ...
        for (let y = 0; y < this.height; y++) {
            ...
            for (let x = 0; x < this.width; x++) {
                row.push(true)//we are passing in all hidden values as true.
            }
            this.hiddenGrid.push(row)
        }
    ...
    render() {
        ...
        for (let y = 0; y < this.grid.length; y++) {
            ...
            for (let x = 0; x < this.grid[y].length; x++) {
                row.push(<Square x={x} y={y} value={this.grid[y][x]} hidden={this.hiddenGrid[y][x]}/>)
            }
